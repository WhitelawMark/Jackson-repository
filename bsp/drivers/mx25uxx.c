 ...
 * ulog cfg
*/
#define LOG_TAG           "drv.mx25"
#define DBG_LVL           DBG_INFO
#include <drv_log.h>

#define W25X_CS_PIN         GET_PIN(B,0)
...
rt_err_t W25QXXEraseSector(rt_uint32_t SectorNum)
{
    ...
    cmd[1] = (rt_uint8_t)(Addr >> 16);/*先发高位在发低位*/
...
rt_err_t W25QXXEraseBlock(rt_uint32_t u32BlockNum)
{
...
#if 1
    rt_uint8_t cmd[4];
    cmd[0] = W25X_BlockErase;/*命令+后面加要读的地址*/
    cmd[1] = (rt_uint8_t)(Addr >> 16);/*先发高位在发低位*/
    cmd[2] = (rt_uint8_t)(Addr >> 8);
    cmd[3] = (rt_uint8_t)(Addr);
#endif
    ...
    HAL_SPI_Transmit(&hspi,(uint8_t *)&cmd[0],4,1000);
...
#if 1
    rt_uint8_t cmd[4];
    cmd[0] = W25X_ReadData;/*命令+后面加要读的地址*/
    cmd[1] = (rt_uint8_t)(addr >> 16);/*先发高位在发低位*/
    cmd[2] = (rt_uint8_t)(addr >> 8);
    cmd[3] = (rt_uint8_t)(addr);
#endif
...
#if 1
    rt_uint8_t cmd[4];
    cmd[0] = W25X_PageProgram;/*命令+后面加要读的地址*/
    cmd[1] = (rt_uint8_t)(WriteAddr >> 16);/*先发高位在发低位*/
    cmd[2] = (rt_uint8_t)(WriteAddr >> 8);
    cmd[3] = (rt_uint8_t)(WriteAddr);
#endif
...
void W25QXXWrite(rt_uint32_t dst, rt_uint8_t* pdata, rt_uint32_t ulen)
{
    ...
    if ((dst % W25QXX_PAGE_SIZE) != 0)
    {
        ...
        cmd[0] = W25X_PageProgram;/*命令+后面加要读的地址*/
        cmd[1] = (rt_uint8_t)(dst >> 16);/*先发高位在发低位*/
        cmd[2] = (rt_uint8_t)(dst >> 8);
        cmd[3] = (rt_uint8_t)(dst);
...
int SFLASH_W25QXX_Init(void)
{
    ...
    rt_kprintf("board: MX25UXX init pass.\r\n");
