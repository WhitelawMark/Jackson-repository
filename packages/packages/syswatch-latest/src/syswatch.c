...
#define SYSWATCH_EVENT_REBOOT           (0x01)
...
typedef struct{
    ...
    syswatch_reboot_callback_t reboot_callback;
}syswatch_data_t;
...
{
    rt_event_send(&sw_data.event, SYSWATCH_EVENT_REBOOT);
}
MSH_CMD_EXPORT_ALIAS(syswatch_reboot, reboot, system reboot)
...
{
    LOG_E("Thread exception, priority = %d, execute system reset", thread->current_priority);
...
static void syswatch_thread_entry(void *parameter)
{
    ...
    while(1)
    {
        result = rt_event_recv(&sw_data.event, SYSWATCH_EVENT_REBOOT,
                               RT_EVENT_FLAG_OR |RT_EVENT_FLAG_CLEAR,
                               RT_TICK_PER_SECOND, &e);
        if(result==RT_EOK){
            switch (e){
                case SYSWATCH_EVENT_REBOOT:
                    rt_kprintf("system reboot \r\n");
                    if(sw_data.reboot_callback){
                        sw_data.reboot_callback();
                    }
...
{
    sw_data.reboot_callback = callback;
}
...
int syswatch_init(void)
{
    ...
    result = rt_event_init(&sw_data.event, "event", RT_IPC_FLAG_FIFO);
    if (result != RT_EOK){
        LOG_E("init static semaphore failed.\n");
        return RT_ERROR;
    }
